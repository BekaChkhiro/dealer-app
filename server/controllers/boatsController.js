...
async function getBoats(req, res) {
  try {
    ...
    if (keyword) {
      conditions.push(`(name ILIKE $${paramIndex} OR identification_code ILIKE $${paramIndex})`);
      params.push(`%${keyword}%`);
      paramIndex++;
    }
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) FROM boats ${whereClause}`,
      params
    );
    const total = parseInt(countResult.rows[0].count);

    const dataResult = await pool.query(
      `SELECT * FROM boats ${whereClause} ORDER BY ${sortBy} ${order} LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`,
      [...params, limit, offset]
    );
...
async function createBoat(req, res) {
  try {
    ...
    if (!name) {
      return res.status(400).json({ error: 1, success: false, message: 'Boat name is required' });
    }
...
async function updateBoat(req, res) {
  try {
    ...
    addField('name', name);
    addField('identification_code', identification_code);
    addField('departure_date', departure_date);
    ...
    params.push(id);
    const result = await pool.query(
      `UPDATE boats SET ${fields.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
      params
    );
